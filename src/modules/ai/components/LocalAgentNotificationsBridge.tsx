...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "Marketing Lab needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "Marketing Lab run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "Marketing Lab finished", "Your task is ready");
    }
  }, [status, error]);
